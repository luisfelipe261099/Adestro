  ...
  return (
    <div className="fixed bottom-20 right-4 z-40 flex h-[28rem] w-[22rem] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-md border border-purple-200 bg-white shadow-2xl lg:bottom-6">
      <header className="flex items-center justify-between border-b border-purple-100 bg-gradient-to-r from-purple-600 to-indigo-600 px-3 py-2 text-white">
        <div>
          <p className="text-[12px] font-bold uppercase tracking-wider text-white/80">Assistente IA</p>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-2">
        {messages.length === 0 ? (
          <div className="space-y-2">
            <p className="rounded-md bg-purple-50 px-3 py-2 text-[12px] text-purple-900">
            ...
            <div className="flex flex-wrap gap-1.5">
              {QUICK_PROMPTS.map((prompt) => (
                <button
                  ...
                  type="button"
                  onClick={() => send(prompt)}
                  className="rounded-full border border-purple-200 bg-purple-50 px-2 py-1 text-[12px] font-semibold text-purple-800 hover:bg-purple-100"
                >
                  {prompt}
                </button>
              ))}
          ...
          messages.map((turn, idx) => (
            ...
            >
              <div
                className={`max-w-[85%] whitespace-pre-wrap rounded-md px-3 py-2 text-[12.5px] leading-snug ${
                  turn.role === "user"
        ...
        {busy ? (
          <p className="rounded-md bg-slate-100 px-3 py-2 text-[12px] text-slate-600">Pensando…</p>
        ) : null}
      </div>
      ...
      >
        ...
        <button
          ...
          className="rounded-full bg-purple-600 px-3 py-1.5 text-[12px] font-bold text-white shadow-sm disabled:opacity-50"
        >
          →
        </button>
      </form>
