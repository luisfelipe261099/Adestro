...
export default function AgendaClientPage() {
  ...
  const [selectedDogId, setSelectedDogId] = useState(
    searchParams.get("dogId") ?? clients[0]?.dogs[0]?.id ?? "",
  );
  ...
  // `?new=true` abre o formulário já na chegada — é o que os botões "Novo
  // agendamento" e "Agendar aula" espalhados pelo app sempre prometeram.
  const [showForm, setShowForm] = useState(searchParams.get("new") === "true");
  const [showStatusFilters, setShowStatusFilters] = useState(false);
  const [agendaMessage, setAgendaMessage] = useState("");
  // Choque de horário recusado pelo servidor, aguardando decisão do adestrador.
  const [pendingConflicts, setPendingConflicts] = useState<EventConflict[] | null>(null);
  ...
  // Compara datas RESOLVIDAS — a comparação literal de string deixava passar
  // eventos gravados como "dd/mm/yyyy" ou como nome de dia da semana.
  ...
  /**
   * Cria o agendamento. `allowOverlap` só vai como `true` quando o adestrador
   * confirma o choque de horário explicitamente — o servidor recusa por padrão.
   */
  async function createEvent(allowOverlap: boolean) {
    ...
    try {
      ...
      if (result.reason === "conflict") {
        // Não cria e não descarta o formulário: pergunta antes.
        setPendingConflicts(result.conflicts);
        return;
      }

      setAgendaMessage(result.message);
